...
namespace IsEmirleri.Business.Concrete
{
    public class MissionService : Service<Mission>, IMissionService
    {
        ...
        public MissionEfficiencyDto GetUserMissionEfficiency(int userId)
        {
            ...
            // Gerçekleşen toplam süre (TotalDuration) - Aggregate kullanarak TimeSpan toplama
            TimeSpan actualTime = userTasks
                .Where(m => m.TotalDuration.HasValue) // TotalDuration değeri olanları filtrele
                .Select(m => m.TotalDuration.Value)   // Nullable TimeSpan'leri TimeSpan'e çevir
                .Aggregate(TimeSpan.Zero, (total, next) => total.Add(next)); // TimeSpan'leri topla
